...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      type: "article",
      locale: "zh_CN",
      url: `https://openclaw2026.cc/topics/${topic.slug}`,
      siteName,
      title: `${topic.title} | ${siteName}`,
      description: topic.description,
    },
...
export default async function TopicPage({ params }: Props) {
  ...
  const quickStartByCategory: Partial<Record<(typeof topic.category), string[]>> = {
    ...
    渠道: ["确认鉴权与回调地址", "检查触发规则（@/关键词）", "核对 DM 配对与 allowFrom 权限", "查看日志定位失败节点"],
    技能: ["先列可用技能并确认来源", "按目录优先级安装和去重", "新增后跑最小回归并记录版本"],
  ...
  const acceptanceByCategory: Partial<Record<(typeof topic.category), string[]>> = {
    ...
    技能: ["技能路径与版本可见", "同名冲突已去重", "新增 skill 调用结果可复现"],
  ...
  const topicUrl = `https://openclaw2026.cc/topics/${topic.slug}`;
  const faqJsonLd = {
